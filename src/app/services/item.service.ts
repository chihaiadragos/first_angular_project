...
export class ItemService {
  ...
  public update(item: Item){
    //request de tip put modifica toate proprietatile obiectului mai putin id
    //request de tip patch modifica doar proprietati pe care le dorim
    const body = {
      id: item.id,
      title: item.title,
      description: item.description,
  ...
  public read(){
    this.httpClient.get(this.apiUrl).subscribe((response: any) => {
      ...
      //response.data este lista de elemente din baza de date
